...
const existingOwnerId = ref('')
const existingStatus = ref('DRAFT')
...
const loadProposalDetail = async () => {
  if (!proposalId.value) {
    submitError.value = 'ID proposal tidak ditemukan.'
  ...
  try {
    const response = await $fetch<any>(`${config.public.apiBase}/api/proposal/${proposalId.value}`, {
    ...
    judul.value = detail?.title || ''
    kategoriProgram.value = detail?.category || ProgramCategory.PENGABDIAN
    focusType.value = detail?.focusType || FocusType.TEMATIK
    if (focusType.value === FocusType.TEMATIK) {
      focusTematikValue.value = detail?.focusValue || ''
      focusRirnValue.value = ''
    }
    else {
      focusRirnValue.value = detail?.focusValue || ''
      focusTematikValue.value = ''
    }

    kelompokSkema.value = detail?.schemeGroup || ''
    ruangLingkup.value = detail?.scopeLevel || ScopeLevel.NASIONAL
    tahunPertamaUsulan.value = detail?.firstProposalYear ? String(detail.firstProposalYear) : ''
    lamaKegiatan.value = detail?.durationMonths ? String(detail.durationMonths) : ''
    rumpunIlmuLevel1.value = detail?.scientificFieldLevel1 || ''
    rumpunIlmuLevel2.value = detail?.scientificFieldLevel2 || ''
    rumpunIlmuLevel3.value = detail?.scientificFieldLevel3 || ''
    namaKetua.value = detail?.leadName || ''
    inputMode.value = detail?.inputMode || ProposalInputMode.EDITOR
    isiProposal.value = detail?.editorContent || '<p>Mulai ketik proposal di sini...</p>'
    existingOwnerId.value = detail?.ownerId || user.value?.id || ''
    existingStatus.value = detail?.status || 'DRAFT'

    dosenRows.value = (detail?.serviceMembers || []).map((member: any, index: number) => ({
      id: member.id || `${Date.now()}-dosen-${index}`,
      nidn: member.nidn || '',
      name: member.name || '',
      duty: member.duty || '',
      status: 'Dosen',
    ...
    mahasiswaRows.value = (detail?.studentMembers || []).map((member: any, index: number) => ({
      id: member.id || `${Date.now()}-mahasiswa-${index}`,
      memberType: 'Mahasiswa',
    ...
    submitError.value = error?.data?.message || 'Gagal memuat detail proposal.'
  }
  finally {
    isLoadingDetail.value = false
  }
...
const submitProposal = async () => {
  ...
  const ownerId = existingOwnerId.value || user.value?.id || ''
  if (!ownerId) {
    submitError.value = 'ownerId tidak ditemukan. Silakan login ulang.'
    return
  }
  ...
  try {
    if (inputMode.value === ProposalInputMode.UPLOAD) {
      ...
      await $fetch(`${config.public.apiBase}/api/proposal/${proposalId.value}`, {
        method: 'PUT',
        headers: accessToken.value
          ? { Authorization: `Bearer ${accessToken.value}` }
          : undefined,
    ...
    submitSuccess.value = 'Proposal berhasil diperbarui.'
    await navigateTo('/proposals')
  }
  catch (error: any) {
    submitError.value = error?.data?.message || 'Gagal memperbarui proposal.'
  }
  finally {
  ...
  <div class="h-screen w-screen bg-gray-100">
    <div class="flex h-full w-full">
      ...
      <main class="flex-1 overflow-y-auto bg-gray-50">
        ...
        <div class="px-7 py-6">
          <h1 class="text-2xl mb-5 font-semibold text-gray-900">Edit Usulan Pengabdian</h1>

          <p v-if="isLoadingDetail" class="mb-4 text-sm text-gray-500">Memuat data proposal...</p>
